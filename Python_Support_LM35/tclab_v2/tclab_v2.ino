...
// return average  of n reads of thermister temperature in °C for TMP35
...
// return average of n reads of LM35 temperature in °C
inline float readTemperature(int pin) {
  float degC = 0.0;
  for (int i = 0; i < n; i++) {
    degC += analogRead(pin) * 0.252265625;      // for 3.3V AREF
    //degC += analogRead(pin) * 0.170898438;    // for 1.75V AREF
  }
  return degC / float(n);
}
